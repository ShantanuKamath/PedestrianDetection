...
template <class T> static inline void swapi(T& x, T& y) { T t=x; x=y; y=t; }
#ifndef min
	...
	while(iter < max_iter)
	{
		...
		for(s=0;s<active_size;s++)
		{
			...
			if(Ai > 0)
			{
				...
				for(m=0;m<active_size_i[i];m++)
				{
					if(be_shrunk(i, m, y_index[i], alpha_i[alpha_index_i[m]], minG))
					{
						...
						while(active_size_i[i]>m)
						{
							...
							{
								swapi(alpha_index_i[m], alpha_index_i[active_size_i[i]]);
								swapi(G[m], G[active_size_i[i]]);
								if(y_index[i] == active_size_i[i])
									y_index[i] = m;
								else if(y_index[i] == m)
				...
				{
					active_size--;
					swapi(index[s], index[active_size]);
					s--;
					continue;
				}
		...
		while(iter < max_iter)
		{
			...
			for(s=0; s<QP_active_size; s++)
			{
				...
				if(wpd[j] == 0)
				{
					...
					{
						QP_active_size--;
						swapi(index[s], index[QP_active_size]);
						s--;
						continue;
					}
				}
...
static void group_classes(const problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	...
	// we swapi labels to ensure that internally the binary SVM has positive data corresponding to the +1 instances.
	//
	if (nr_class == 2 && label[0] == -1 && label[1] == 1)
	{
		swapi(label[0],label[1]);
		swapi(count[0],count[1]);
		for(i=0;i<l;i++)
		{
			if(data_label[i] == 0)
				data_label[i] = 1;
